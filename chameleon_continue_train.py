...
from chameleon_train import define_argparser
from chameleon_train import main
...
def overwrite_config(config, prev_config):
    ...
    for prev_key in vars(prev_config).keys():
        if prev_key not in vars(config).keys():
            ...
            print(
                'WARNING!!! Argument "--%s" is not found in current argument parser.\tIgnore saved value:' % prev_key,
                vars(prev_config)[prev_key],
            )

    for key in vars(config).keys():
        if not key not in vars(prev_config).keys():
            # No such argument in saved file. Use current value.
            print(
                'WARNING!!! Argument "--%s" is not found in saved model.\tUse current value:' % key, vars(config)[key]
            )
...
def continue_main(config, main):
    ...
    if os.path.isfile(config.load_fn):
        ...
        # torch will load model automatically to the gpu number which was used in the training before.
        # But if we want to use another gpu number for some reasons, we have to specify the model number.
