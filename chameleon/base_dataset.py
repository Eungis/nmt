...
class Vocabulary(object):
    ...
    def __init__(self, min_freq=1, max_vocab=99999):
    ...
    def build_vocab(self, sents, delimiter=" "):
        ...
        log.info(f"Number of vocabularies: {len(self)}")
    ...
    def decode(self, indice, delimiter=" ", removed_indice=[BOS, EOS, PAD]):
        ...
        if is_nested:
            ...
            for encoded_text in indice:
                ...
                decoded_indice += [delimiter.join(decoded)]
...
class TranslationCollator:
    ...
    def __call__(self, batch):
        ...
        if self.with_text:
            ...
            for sample in batch:
                srcs.append((sample["src"][: self.max_length], len(sample["src"][: self.max_length])))
                tgts.append((sample["tgt"][: self.max_length], len(sample["tgt"][: self.max_length])))

                srcs_texts.append(" ".join(sample["src_text"].split(" ")[: self.max_length]))
                tgts_texts.append(" ".join(sample["tgt_text"].split(" ")[: self.max_length]))
