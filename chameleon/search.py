...
class BeamSearchBoard:
    def __init__(
        ...
        self.word_indice = [torch.LongTensor(beam_size).zero_().to(self.device) + Vocabulary.BOS]
        ...
        self.cumulative_probs = [torch.FloatTensor([0.0] + [-float("inf")] * (beam_size - 1)).to(self.device)]
    ...
    def collect_result(self, y_hat, prev_status):
        ...
        self.masks += [torch.eq(self.word_indice[-1], Vocabulary.EOS)]  # Set finish mask if we got EOS.
        ...
        for prev_status_name, prev_status in prev_status.items():
            self.prev_status[prev_status_name] = torch.index_select(
                prev_status, dim=self.batch_dims[prev_status_name], index=self.beam_indice[-1]
            ).contiguous()

    def get_n_best(self, n=1, length_penalty=0.2):
