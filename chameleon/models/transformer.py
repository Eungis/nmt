...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x, key_and_value, mask, prev, future_mask):
        ...
        if prev is None:  # Training mode
            ...
            z = x + self.masked_attn_dropout(self.masked_attn(z, z, z, mask=future_mask))
        else:  # Inference mode
            ...
            z = self.masked_attn_norm(x)
            z = x + self.masked_attn_dropout(self.masked_attn(z, normed_prev, normed_prev, mask=None))
        ...
        z = z + self.attn_dropout(
            self.attn(Q=self.attn_norm(z), K=normed_key_and_value, V=normed_key_and_value, mask=mask)
        )
        # |z| = (batch_size, m, hidden_size)
...
class Transformer(nn.Module):
    ...
    def search(self, x, is_greedy=True, max_length=255):
        ...
        y_t_1 = x.new(batch_size, 1).zero_() + Vocabulary.BOS
        # |y_t_1| = (batch_size, 1)
        ...
        while is_decoding.sum() > 0 and len(indice) < max_length:
            ...
            h_t = self.emb_dropout(self._position_encoding(self.emb_dec(y_t_1), init_pos=len(indice)))
            ...
            is_decoding = is_decoding * torch.ne(y_t_1, Vocabulary.EOS)
            # |y_t_1| = (batch_size, 1)
    ...
    def batch_beam_search(
        ...
        while sum(done_cnt) < batch_size and length <= max_length:
            ...
            # Unlike training procedure,
            # take the last time-step's output during the inference.
            h_t = self.emb_dropout(self._position_encoding(self.emb_dec(fab_input), init_pos=length))
