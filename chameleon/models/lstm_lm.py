...
class LanguageModel(nn.Module):
    def __init__(self, vocab_size, word_vec_size=256, hidden_size=512, n_layers=4, dropout=0.3, max_length=255):
    ...
    def search(self, batch_size=64, max_length=255):
        x = torch.LongTensor(batch_size, 1).to(next(self.parameters()).device).zero_() + Vocabulary.BOS
        # |x| = (batch_size, 1)
        ...
        while is_undone.sum() > 0 and len(indice) < max_length:
            ...
            y = y.masked_fill_((1.0 - is_undone).byte(), Vocabulary.PAD)
            is_undone = is_undone * torch.ne(y, Vocabulary.EOS).float()
            # |y| = (batch_size, 1)
