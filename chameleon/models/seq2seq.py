...
from chameleon.search import BeamSearchBoard
...
class Seq2Seq(nn.Module):
    ...
    def fast_merge_encoder_hiddens(self, encoder_hiddens):
        ...
        h_0_tgt = (
            h_0_tgt.transpose(0, 1).contiguous().view(batch_size, -1, self.hidden_size).transpose(0, 1).contiguous()
        )
        c_0_tgt = (
            c_0_tgt.transpose(0, 1).contiguous().view(batch_size, -1, self.hidden_size).transpose(0, 1).contiguous()
        )
    ...
    def search(self, src, is_greedy=True, max_length=255):
        ...
        y = x.new(batch_size, 1).zero_() + Vocabulary.BOS
        ...
        while is_decoding.sum() > 0 and len(indice) < max_length:
            ...
            y = y.masked_fill_(~is_decoding, Vocabulary.PAD)
            # Update is_decoding if there is EOS token.
            is_decoding = is_decoding * torch.ne(y, Vocabulary.EOS)
            # |is_decoding| = (batch_size, 1)
            indice += [y]
            # |indice| = [(batch_size, 1), (batch_size, 1), ...]
    ...
    # @profile
    def batch_beam_search(self, src, beam_size=5, max_length=255, n_best=1, length_penalty=0.2):
        ...
        # initialize 'BeamSearchBoard' as many as batch_size
        boards = [
            BeamSearchBoard(
                h_src.device,
                {
        ...
        for sentence in batch_sentences:
            # sentence = [[tensor([idx1]), tensor([idx2]), ...]]
            sentence = torch.stack(sentence[0]).tolist()
            # sentence = [idx1, idx2, ...]
            indice += [sentence]

        return indice, batch_probs
